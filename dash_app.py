...
class Plotter():
    ...
    def make_song_info(self, idx, df):
        ...
        # fig_moods.update_traces(fill='toself')
        ...
        layout = html.Div([
            ...
            html.Div(children=[
                dcc.Graph(figure=fig_quads,  style={'height': '40vh'}),
            ], className='three columns'),

            html.Div(children=[
                dcc.Graph(figure=fig_moods,  style={'height': '40vh'}),
            ], className='nine columns'),
            ], className='twelve columns'),

            html.Div(children=[
                dcc.Graph(figure=fig_cld), #,  style={'height': '40vh'}),
            ], className='twelve columns'),
            # 
        ], className='twelve columns')
    ...
    def create_layout(self, app):
        ...
        layout = html.Div(style={'background-color':'#ffffff'}, children=[
            ...
            html.H3(children='MTG-MER: an open data set for personalized Music Emotion Recognition'),
            # html.Img(src='/img/mtg.png'),

            html.Section(className='row header', style={'padding':'5px'}, children=[
                html.Div(children=[
            ...
            html.Div(className='row', children=[
                ...
                html.Div(children=[
                    dcc.Graph(id='graph-arousal-valence',  style={'height': '70vh'}),
    ...
    def run_callbacks(self, app):
        ...
        def display_plot(av_rep, spoti_filt, sl_mode, sl_key, sl_tempo, sl_users):
            ...
            list_opacity = np.sqrt(this_df.num_users / np.max(this_df.num_users))
            data = []
            for sel, group in this_df.groupby('selection'):
                list_opacity = np.sqrt(group.num_users / np.max(group.num_users))
